...
class TextLine extends StatelessWidget {
  ...
  TextStyle _getInlineTextStyle(NotusStyle style, ZefyrThemeData? theme) {
    ...
    if (style.contains(NotusAttribute.fontSize)) {
      result =
          result.merge(TextStyle(fontSize: style.get(NotusAttribute.fontSize)!.value!.toDouble()));
    }
    ...
    if (style.contains(NotusAttribute.backgroundColor)) {
      result = result.merge(
          TextStyle(backgroundColor: Color(style.get(NotusAttribute.backgroundColor)!.value!)));
    }
    if (style.contains(NotusAttribute.underline)) {
      if (style.contains(NotusAttribute.color)) {
        var color = Color(style.get(NotusAttribute.color)!.value!);
        result = _mergeTextStyleWithDecoration(
          result,
          TextStyle(decoration: TextDecoration.underline, decorationColor: color),
        );
      } else {
    ...
    if (style.contains(NotusAttribute.strikethrough)) {
      if (style.contains(NotusAttribute.color)) {
        ...
        result = _mergeTextStyleWithDecoration(
          result,
          TextStyle(decoration: TextDecoration.lineThrough, decorationColor: color),
        );
      } else {
